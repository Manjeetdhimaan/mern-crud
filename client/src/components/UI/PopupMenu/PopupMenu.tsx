...
const PopupMenu: React.FC<IPopupMenuProps> = ({ items, data }) => {
  ...
  useEffect(() => {
    const updateMenuPosition = () => {
      if (buttonRef.current) {
        ...
        if (spaceBelow < 150 && spaceAbove > 150) { // Assuming the menu height is around 150px
          setMenuPosition('top');
        } else {
  ...
  const handleItemClick = (label: string, onClick: <T>(label: string, data?: T) => void) => {
    onClick(label, data);
  ...
  const renderMenuItems = (items: IMenuItem[]) => {
    return items.map((item, index) => (
      <li key={index} onClick={() => handleItemClick(item.label, item.onClick)} className="menu-item">
  ...
  return (
    <div className="menu-container">
      <a className="menu-button cursor-pointer" onClick={toggleMenu} ref={buttonRef}>
        <ArrowDownIcon />
      </a>
      {isOpen && (
