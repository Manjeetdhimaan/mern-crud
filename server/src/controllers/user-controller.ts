...
export default class UserController {
    ...
    getUserWithRole = async (req: IRequest, res: Response, next: NextFunction): Promise<Response | void> => {
        try {
            ...
            const { getDeleted } = req.query;
            // const fields = 'id, fullName, email, createdAt, updatedAt, isDeleted, roleId'
            const data = await this.userService.getUsersWithRole(Number(id), getDeleted === "true" ? true : false) as IUser[];
            if (data && data.length > 0) {
                // const { password, ...rest } = data[0];
                const user = data[0];
                const userWithRole = {
                ...
                return res.status(200).json(successAction({ user: userWithRole }, "User fetched successfully!"));
            }
            return res.status(200).json(successAction(null, "No user found"));
        } catch (error) {
            console.log(error);
    ...
    getUsersWithRole = async (req: IRequest, res: Response, next: NextFunction): Promise<Response | void> => {
        try {
            ...
            const { getDeleted } = req.query;
            // const fields = 'id, fullName, email, createdAt, updatedAt, isDeleted, roleId'
            const data = await this.userService.getUsersWithRole(undefined, getDeleted === "true" ? true : false) as IUser[];
            if (data && data.length > 0) {
                // const { password, ...rest } = data[0];
