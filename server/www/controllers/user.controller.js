    ...
    constructor() {
        ...
        this.getUsers = async (req, res, next) => {
            try {
                ...
                const data = await this.databaseService.getAll(database_tables_1.USERS, fields, (Number(page) || 1), (Number(perPage) || 10), undefined, searchQuery, getDeleted === "true" ? true : false);
                if (!data || data.length <= 0)
        ...
        this.getUser = async (req, res, next) => {
            try {
                ...
                const data = await this.databaseService.getData(database_tables_1.USERS, 'id', Number(id), fields, undefined, undefined, undefined, getDeleted === "true" ? true : false);
                if (data && data.length > 0) {
